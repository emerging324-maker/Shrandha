...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${scriptUrl()}?action=listBatches&secret=${encodeURIComponent(apiSecret())}`, {
      cache: "no-store",
    });
...
// POST — create a new batch: { name, domain }
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await fetch(scriptUrl(), {
      ...
      body: JSON.stringify({ action: "createBatch", ...body, secret: apiSecret() }),
    });
    const data = await res.json();
    return NextResponse.json(data);
  } catch {
